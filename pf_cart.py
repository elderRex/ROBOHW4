...
class robot:
    ...
    def __init__(self):
        ...
        self.forward_cnt = 10
        self.part_forward_dis = 10 * 1.04
        self.car_axis = 11.55
        self.r = self.car_axis / 2
        self.perimeter = 20.4
    ...
    '''
    def get_rotate_target(self,ori_rot):
        target = int(ori_rot * self.r * 18 / self.perimeter)
        print "target" + str(target)
        return target

    def turn_to_angle(self,ori_rot):
        # the number of encoders will rotate while it is rotating itself in place
        print "rotating"
        target = self.get_rotate_target(ori_rot)
        enc_tgt(1, 1, target)
        set_speed(180)
        right_rot()
    ...
    def check_obstacle(self,oz):
        ...
        while oz[4] <= self.collision_dist + sqrt(2 * 5.7 * 5.7):
            ...
            orientation %= 2 * pi
            self.orientation = orientation #virtual car has turned
            self.turn_to_angle(turn) # turn the real cart
            oz = self.real_sense()
        res.append(turn)
    ...
    def real_move(self,turn,forward):
        #cart move with forward_cnt 10 encoder count
...
worldx = input('world x')
worldy = input('world y')
world_size[0] = worldx
world_size[1] = worldy
...
real_x = input('cart x')
real_y = input('cart y')
real_ori = input('cart ori')

my_robot.set(real_x,real_y,real_ori)
...
for t in range(T):
    ...
    '''
    init_sense = my_robot.real_sense()
    print init_sense
    print my_robot
    movement_spec = my_robot.check_obstacle(init_sense)
    #ready to go
    print movement_spec[0]
    my_robot = my_robot.real_move(movement_spec[0], movement_spec[1])
    Z = my_robot.real_sense()
